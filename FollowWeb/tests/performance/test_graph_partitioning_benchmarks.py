...
def benchmark_merge(
    results: List, original_graph: nx.DiGraph
) -> Tuple[float, float]:
    """
...
class TestPartitioningPerformance:
    ...
    def test_verify_1m_nodes_under_30_minutes(self):
        ...
        assert estimated_total * 2 < 1800, f"Estimated time too slow: {estimated_total*2:.2f}s"
    ...
    def test_benchmark_full_pipeline_100k(self):
        ...
        assert total_time < 300, f"Pipeline too slow: {total_time:.2f}s"  # Under 5 minutes
        assert report["throughput"] > 300, f"Throughput too low: {report['throughput']:.0f} nodes/s"
    ...
    def test_benchmark_full_pipeline_300k(self):
        ...
        print("=" * 60)
        print(f"Graph size: {report['graph_size']:,} nodes")
        print(f"Partitions: {report['num_partitions']} (analyzed {report['partitions_analyzed']})")
        print(f"Partition time: {report['partition_time']:.2f}s")
        ...
        # Verify reasonable performance
        assert total_time < 600, f"Pipeline too slow: {total_time:.2f}s"  # Under 10 minutes
    ...
    def test_generate_performance_report(self):
        ...
        print("=" * 80)
        print(f"{'Nodes':<12} {'Partitions':<12} {'Partition':<12} {'Analysis':<12} {'Merge':<12} {'Total':<12}")
        print("-" * 80)
...
class TestScalabilityAnalysis:
    ...
    def test_partition_time_scaling(self):
        ...
        for i in range(1, len(sizes)):
            ...
            assert time_ratio < size_ratio ** 1.5, f"Scaling too poor: {time_ratio:.1f}x for {size_ratio:.1f}x size"
    ...
    def test_merge_time_scaling(self):
        ...
        for i in range(1, len(partition_counts)):
            ...
            print(f"Partition ratio: {partition_ratio:.1f}x, Time ratio: {time_ratio:.1f}x")
            
            # Time should scale linearly or better
            assert time_ratio <= partition_ratio * 1.5, f"Merge scaling too poor: {time_ratio:.1f}x"
