...
def run_benchmarks() -> int:
    ...
    cmd = [
        ...
        '-n', '0',  # Force sequential execution
...
def run_all_tests_optimally(extra_args: List[str]) -> int:
    ...
    try:
        ...
        if benchmark_result != 0:
            ...
            print(f"❌ Benchmark tests failed with exit code: {benchmark_result}")
        else:
        ...
        # Final summary
        if total_failures == 0:
            print("🎉 All tests passed! (Regular tests: parallel, Benchmark tests: sequential)")
        else:
            print(f"❌ Some tests failed (Total failure codes: {total_failures})")
...
def run_tests_safely(test_args: List[str], test_type: Optional[str] = None) -> int:
    ...
    if test_type is None:
        if any('-m' in test_args and 'benchmark' in test_args[i+1] 
               for i, arg in enumerate(test_args[:-1]) if arg == '-m'):
            test_type = "benchmark"
        elif any('-m' in test_args and 'unit' in test_args[i+1] 
                 for i, arg in enumerate(test_args[:-1]) if arg == '-m'):
            test_type = "unit"
        elif any('-m' in test_args and 'integration' in test_args[i+1] 
                 for i, arg in enumerate(test_args[:-1]) if arg == '-m'):
            test_type = "integration"
        elif any('-m' in test_args and ('slow' in test_args[i+1] or 'performance' in test_args[i+1])
                 for i, arg in enumerate(test_args[:-1]) if arg == '-m'):
...
def main():
    ...
    if command == "system-info":
        ...
        print(f"  Memory: {info['memory_gb']:.1f} GB total, {info['available_memory_gb']:.1f} GB available")
