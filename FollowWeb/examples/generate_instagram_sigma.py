...
def load_instagram_data(json_path: str) -> nx.DiGraph:
    ...
    for user_data in data:
        ...
        graph.add_node(
            user, followers_count=followers_count, following_count=following_count
        )
...
def main():
    ...
    renderer = SigmaRenderer(
        vis_config=vis_config, template_name="sigma_instagram.html"
    )

    # Generate visualization
    output_path = (
        Path(__file__).parent.parent / "Output" / "instagram_network_sigma.html"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print("Generating visualization...")
    success = renderer.generate_visualization(
        graph=graph, output_filename=str(output_path)
    )
