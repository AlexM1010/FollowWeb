"""
Matplotlib renderer for static PNG network visualizations.

This module provides the MatplotlibRenderer class that implements the Renderer interface
...
class MatplotlibRenderer(Renderer):
    ...
    def __init__(
        ...
        """
        Initialize the matplotlib renderer with visualization configuration.
    ...
    def generate_visualization(
        ...
        """
        Generate static PNG visualization from graph using matplotlib.
        ...
        if metrics and metrics.layout_positions:
            self.logger.info("Using shared layout positions for PNG generation")
            pos = metrics.layout_positions
        else:
            ...
            self._add_edge_weights_for_layout(G_undirected, metrics.edge_metrics)
        ...
        with ProgressTracker(
            ...
            # Use metrics for edge rendering
            for (u, v), edge_metric in metrics.edge_metrics.items():
                hex_color = edge_metric.color.lstrip("#")
                mpl_width = edge_metric.width * 0.8
            ...
            # Step 4: Draw nodes
            node_colors = [
                metrics.node_metrics[node].color_rgba for node in graph.nodes()
            ]
            node_sizes = [metrics.node_metrics[node].size for node in graph.nodes()]
    ...
    def _get_effective_layout_type(self) -> str:
        ...
        # Get configured layout type, defaulting to spring
    ...
    def _calculate_layout(
        ...
        """
        Calculate layout positions for all supported layout types.
    ...
    def _calculate_spring_layout(
        ...
        fast_mode = self.performance_config.get("fast_mode", False)
    ...
    def _run_chunked_spring_layout(
        ...
        chunk_size = max(1, iterations // 10)
    ...
    def _calculate_circular_layout(
        ...
        """Calculate circular layout with community grouping."""
        png_config = self.vis_config.get("png_layout", {})
        circular_config = (
    ...
    def _calculate_shell_layout(
        ...
        """Calculate shell layout with community-based shells."""
        png_config = self.vis_config.get("png_layout", {})
        shell_config = (
    ...
    def _create_community_shells(
        ...
        return [shell for shell in nodelist if shell]

    def _get_spring_k_value(
        self, spring_config: dict[str, Any], num_nodes: int, density: float
    ) -> float:
        """Get spring k-value with adaptive scaling."""
    ...
    def _calculate_adaptive_k_value_from_base(
        ...
        return min(max(adaptive_k, 0.1), 2.0)

    def get_file_extension(self) -> str:
        """Return file extension for PNG output."""
        return ".png"
