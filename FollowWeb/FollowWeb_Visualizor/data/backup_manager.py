...
class BackupManager:
    ...
    def __init__(
        ...
        if self.enable_tiered:
            for tier_name in ['frequent', 'moderate', 'milestone', 'daily']:
                (self.backup_dir / 'backups' / tier_name).mkdir(parents=True, exist_ok=True)
        else:
    ...
    def create_backup(
        ...
        try:
            ...
            topology_backup = backup_subdir / f'graph_topology_backup_{nodes}nodes_{timestamp}.gpickle'
            metadata_backup = backup_subdir / f'metadata_cache_backup_{nodes}nodes_{timestamp}.db'
            ...
            self._log_info(
                EmojiFormatter.format('package', f"Backup created [{tier}]: {nodes} nodes, {size_mb:.2f} MB")
            )
    ...
    def _cleanup_old_backups(self) -> None:
        ...
        try:
            ...
            keep_recent = set(b['timestamp'] for b in all_backups[:3])
            
            # Filter out protected backups
            backups_to_remove = [
                b for b in backups_to_remove
                if b['timestamp'] not in keep_recent
            ]
            ...
            if removed_count > 0:
                self._save_manifest()
                self._log_info(EmojiFormatter.format('broom', f"Cleaned up {removed_count} old backups"))
    ...
    def _compress_old_backups(self) -> None:
        ...
        """
        try:
            cutoff_date = datetime.now(timezone.utc) - timedelta(days=self.compression_age_days)
            compressed_count = 0
            ...
            for backup in self.manifest['backups']:
                ...
                for file_key in ['topology', 'metadata']:
                    ...
                    # Update manifest
                    backup['files'][file_key] = str(compressed_path.relative_to(self.backup_dir))
            ...
            if compressed_count > 0:
                self._save_manifest()
                self._log_info(EmojiFormatter.format('compress', f"Compressed {compressed_count} old backups"))
