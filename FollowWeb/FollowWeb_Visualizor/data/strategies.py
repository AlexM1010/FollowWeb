"""
Graph filtering and transformation strategies for FollowWeb network analysis.

This module provides the GraphStrategy class containing methods for graph manipulation
operations such as k-core pruning, reciprocity filtering, and ego-alter network creation.
These strategies are applied during the pipeline's strategy phase.
"""
...
class GraphStrategy:
    ...
    def prune_graph(self, graph: nx.DiGraph, min_degree: int) -> nx.DiGraph:
        ...
        self.logger.info(
            f"✅ Final pruned graph: {pruned_graph.number_of_nodes():,} nodes, "
            f"{pruned_graph.number_of_edges():,} edges"
        )
    ...
    def filter_by_reciprocity(self, graph: nx.DiGraph) -> nx.DiGraph:
        ...
        self.logger.info(
            f"✅ Filtered for mutuals: {reciprocal_graph.number_of_nodes():,} nodes, "
            f"{reciprocal_graph.number_of_edges():,} edges"
        )
    ...
    def create_ego_alter_graph(
        ...
        self.logger.info(
            f"✅ Alter graph created: {alter_graph.number_of_nodes():,} alters, "
            f"{alter_graph.number_of_edges():,} connections between them"
        )
        return alter_graph
