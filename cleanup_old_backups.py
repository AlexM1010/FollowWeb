...
def cleanup_old_backups(checkpoint_dir: str, max_backups: int, retention_days: int, logger: logging.Logger) -> int:
    """
    ...
    if not backup_dir.exists():
        logger.warning(EmojiFormatter.format('warning', f"Checkpoint directory does not exist: {checkpoint_dir}"))
        return 0
        ...
        if backup_file.stat().st_mtime < cutoff_time:
            ...
            if safe_file_cleanup(str(backup_file)):
                ...
                warning_msg = EmojiFormatter.format(
                    'warning',
                    f"Failed to delete backup: {backup_file.name}"
                )
