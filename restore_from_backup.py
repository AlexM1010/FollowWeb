...
def list_backups(manager: BackupManager, tier: str | None = None) -> None:
    ...
    for backup in backups:
        ...
        short_timestamp = timestamp.split('T')[0].replace('-', '') + '_' + timestamp.split('T')[1][:6].replace(':', '')
...
def restore_backup(
    ...
    try:
        ...
        print(f"Checkpoint restored: {backup['nodes']:,} nodes, {backup['edges']:,} edges")
        return True
...
def main() -> int:
    ...
    manager = BackupManager(
        backup_dir=args.checkpoint_dir,
        config={},
        logger=logger
    )
