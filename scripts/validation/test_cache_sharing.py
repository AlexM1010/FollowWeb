...
class CacheSharingTester:
    ...
    def __init__(self, checkpoint_dir: Path, cache_dir: Path):
        ...
        self.results = {"tests_passed": 0, "tests_failed": 0, "tests": []}
    ...
    def test_checkpoint_exists(self) -> bool:
        ...
        if missing_files:
            self._record_failure(
                test_name, f"Missing files: {', '.join(missing_files)}"
            )
            return False
    ...
    def test_cache_save(self) -> bool:
        ...
        try:
            ...
            for filename in self.REQUIRED_FILES:
                ...
                if not src.exists():
                    self._record_failure(test_name, f"Source file missing: {filename}")
                    return False
    ...
    def test_cache_restore(self) -> bool:
        ...
        try:
            ...
            for filename in self.REQUIRED_FILES:
                ...
                if not file_path.exists():
                    self._record_failure(
                        test_name, f"Missing file in cache: {filename}"
                    )
                    return False
                ...
                try:
                    with open(file_path, "rb") as f:
                        ...
                        if len(data) == 0:
                            self._record_failure(
                                test_name, f"Empty file in cache: {filename}"
                            )
                            return False
                except Exception as e:
                    self._record_failure(
                        test_name, f"Cannot read file from cache: {filename} - {e}"
                    )
                    return False
        ...
        try:
            ...
            for filename in self.REQUIRED_FILES:
                ...
                with open(original, "rb") as f1, open(cached, "rb") as f2:
                    if f1.read() != f2.read():
                        self._record_failure(
                            test_name, f"Content mismatch for {filename}"
                        )
                        return False
        ...
        try:
            ...
            for filename in self.REQUIRED_FILES:
                ...
                if not file_path.exists() or file_path.stat().st_size == 0:
                    self._record_failure(
                        test_name, f"Repair workflow cannot access: {filename}"
                    )
                    return False
    ...
    def test_validation_workflow_restore(self) -> bool:
        ...
        try:
            ...
            if not cache_checkpoint_dir.exists():
                self._record_failure(
                    test_name, "Cache not found for validation workflow"
                )
                return False
            ...
            for filename in self.REQUIRED_FILES:
                ...
                if not file_path.exists() or file_path.stat().st_size == 0:
                    self._record_failure(
                        test_name, f"Validation workflow cannot access: {filename}"
                    )
                    return False

            self._record_success(
                test_name, "Validation workflow can restore from cache"
            )
            return True
    ...
    def _record_success(self, test_name: str, message: str):
        ...
        self.results["tests"].append(
            {"name": test_name, "status": "PASSED", "message": message}
        )
    ...
    def _record_failure(self, test_name: str, message: str):
        ...
        self.results["tests"].append(
            {"name": test_name, "status": "FAILED", "message": message}
        )
    ...
    def print_results(self):
        ...
        print("-" * 80)
        print(
            f"Total Tests: {self.results['tests_passed'] + self.results['tests_failed']}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test cache sharing between workflows")
    parser.add_argument(
        "checkpoint_dir",
        type=Path,
        nargs="?",
