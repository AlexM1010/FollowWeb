...
class CacheIntegrityVerifier:
    ...
    def calculate_checksum(self, file_path: Path, algorithm: str = 'sha256') -> str:
        ...
        try:
            ...
            self.results['errors'].append(f"Failed to calculate checksum for {file_path.name}: {e}")
            return ''
    ...
    def verify_file(self, filename: str) -> Dict[str, any]:
        ...
        try:
            ...
            self.results['errors'].append(f"Failed to check readability of {filename}: {e}")
            return file_info
    ...
    def verify_checkpoint_metadata(self) -> bool:
        ...
        try:
            ...
            if not has_nodes:
                self.results['warnings'].append("Checkpoint metadata missing node count field")
                return False
            
            if not has_edges:
                self.results['warnings'].append("Checkpoint metadata missing edge count field")
                return False
            
            if not has_timestamp:
                self.results['warnings'].append("Checkpoint metadata missing timestamp field")
                return False
            ...
            self.results['errors'].append(f"Invalid JSON in checkpoint_metadata.json: {e}")
            return False
        except Exception as e:
            self.results['errors'].append(f"Failed to read checkpoint_metadata.json: {e}")
            return False
    ...
    def verify(self) -> bool:
        ...
        if not self.checkpoint_dir.exists():
            self.results['errors'].append(f"Checkpoint directory not found: {self.checkpoint_dir}")
            return False
