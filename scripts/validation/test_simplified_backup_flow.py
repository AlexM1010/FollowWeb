...
def test_backup_manager_simplified():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert not hasattr(manager, 'enable_compression'), "Should not have enable_compression"
        ...
        assert not hasattr(manager, 'get_release_tag'), "Should not have get_release_tag"
        assert not hasattr(manager, '_determine_tier'), "Should not have _determine_tier"
        assert not hasattr(manager, '_cleanup_old_backups'), "Should not have _cleanup_old_backups"
        assert not hasattr(manager, '_compress_old_backups'), "Should not have _compress_old_backups"
        assert not hasattr(manager, 'list_backups'), "Should not have list_backups"
        assert not hasattr(manager, 'get_backup_stats'), "Should not have get_backup_stats"
        assert not hasattr(manager, 'backup_intervals'), "Should not have backup_intervals (no tiers)"
...
def test_backup_interval():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        manager = BackupManager(
            backup_dir=tmpdir,
            config={'backup_interval_nodes': 100},
            logger=None
        )
...
def test_workflow_separation():
    ...
    collection_workflow = (project_root / ".github/workflows/freesound-nightly-pipeline.yml").read_text(encoding='utf-8')
    ...
    download_checkpoint_section = "Download checkpoint from private repository" in collection_workflow
    ...
    backup_workflow = (project_root / ".github/workflows/freesound-backup.yml").read_text(encoding='utf-8')
    ...
    assert "node_count % 100 == 0" in backup_workflow or "backup_interval=100" in backup_workflow, \
        "Backup workflow should have 100-node backup interval"
    assert "v-checkpoint" in backup_workflow, "Backup workflow should use v-checkpoint release"
    ...
    assert "v-permanent" not in backup_workflow, "Backup workflow should not use v-permanent release"
...
def test_documentation():
    ...
    backup_manager_code = (project_root / "FollowWeb/FollowWeb_Visualizor/data/backup_manager.py").read_text(encoding='utf-8')
    
    assert "Single source of truth" not in backup_manager_code or \
           "backup workflow" in backup_manager_code.lower(), \
        "BackupManager should reference backup workflow"
    assert "Tier determination" in backup_manager_code or \
           "tier determination" in backup_manager_code.lower(), \
