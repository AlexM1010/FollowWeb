...
class WorkflowVerifier:
    ...
    def trigger_nightly_collection(
        ...
        try:
            ...
            cmd = [
                ...
                "--repo", self.repo,
                "--ref", "main",
                "--field", f"max_requests={max_requests}",
                "--field", f"discovery_mode={discovery_mode}"
            ]
    ...
    def verify_cache_save(self, run_id: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            cache_saved = "Cache saved successfully" in logs or "actions/cache/save" in logs
    ...
    def verify_downstream_workflows(
        ...
        if repair_run_id:
            ...
            if results["repair"]["success"]:
                ...
                validation_run_id = self._get_latest_run_id("freesound-validation-visualization.yml")
                if validation_run_id:
                    results["validation"]["triggered"] = True
                    results["validation"]["run_id"] = validation_run_id
                    
                    success, conclusion, _ = self.monitor_workflow_run(validation_run_id, timeout)
                    ...
                    if results["validation"]["success"]:
                        ...
                        if backup_run_id:
                            ...
                            success, conclusion, _ = self.monitor_workflow_run(backup_run_id, timeout)
    ...
    def verify_visualization_deployed(self) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            cmd = [
                ...
                "--jq", ".[0] | {sha: .sha, message: .commit.message, date: .commit.author.date}"
            ]
    ...
    def generate_report(
        ...
        all_success = (
            ...
            checkpoint_upload.get("upload_success", False) and
            cache_save.get("cache_saved", False) and
            downstream_results.get("repair", {}).get("success", False) and
            downstream_results.get("validation", {}).get("success", False) and
            downstream_results.get("backup", {}).get("success", False) and
            visualization.get("is_visualization_commit", False)
        )
        ...
        if collection_result.get("success"):
            ...
            report_lines.append(f"- ❌ Status: {collection_result.get('conclusion', 'failed')}")
        ...
        if checkpoint_upload.get("upload_success"):
            ...
            report_lines.append(f"- ✅ Verification: {'Passed' if checkpoint_upload.get('verification_passed') else 'Failed'}")
        else:
        ...
        if cache_save.get("cache_saved"):
            ...
            report_lines.append(f"- {'✅' if cache_save.get('has_topology') else '❌'} graph_topology.gpickle")
            report_lines.append(f"- {'✅' if cache_save.get('has_metadata_db') else '❌'} metadata_cache.db")
            report_lines.append(f"- {'✅' if cache_save.get('has_checkpoint_meta') else '❌'} checkpoint_metadata.json")
        else:
        ...
        if repair.get("triggered"):
            report_lines.append(f"- ✅ Triggered: Yes (Run ID: `{repair.get('run_id')}`)")
            report_lines.append(f"- {'✅' if repair.get('success') else '❌'} Success: {repair.get('success')}")
        else:
        ...
        if validation.get("triggered"):
            report_lines.append(f"- ✅ Triggered: Yes (Run ID: `{validation.get('run_id')}`)")
            report_lines.append(f"- {'✅' if validation.get('success') else '❌'} Success: {validation.get('success')}")
        else:
            report_lines.append("- ❌ Triggered: No")
        ...
        if backup.get("triggered"):
            report_lines.append(f"- ✅ Triggered: Yes (Run ID: `{backup.get('run_id')}`)")
            report_lines.append(f"- {'✅' if backup.get('success') else '❌'} Success: {backup.get('success')}")
        else:
            report_lines.append("- ❌ Triggered: No")
        ...
        if visualization.get("is_visualization_commit"):
            ...
            report_lines.append(f"- Commit: `{visualization.get('latest_commit', 'N/A')[:7]}`")
...
def main():
    ...
    parser.add_argument(
        "--check-status",
        metavar="RUN_ID",
        help="Check status of a specific run"
    )
    ...
    try:
        ...
        if args.trigger_collection:
            ...
            if success:
                ...
                logger.info(f"Monitor with: python {sys.argv[0]} --check-status {run_id}")
                sys.exit(0)
            else:
            ...
            success, conclusion, details = verifier.monitor_workflow_run(args.check_status)
            ...
            all_success = (
                ...
                downstream_results.get("repair", {}).get("success", False) and
                downstream_results.get("validation", {}).get("success", False) and
                downstream_results.get("backup", {}).get("success", False)
            )
