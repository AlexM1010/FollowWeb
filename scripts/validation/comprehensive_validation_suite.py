...
def run_actionlint(workflow_files: List[Path]) -> Tuple[List[str], List[str], List[str]]:
...
def validate_workflow(workflow_path: Path, all_errors: List[str], all_warnings: List[str], all_notes: List[str]) -> WorkflowValidation:
...
def print_report(report: ValidationReport):
    ...
    for workflow in report.workflows:
        status_icon = "✓" if workflow.status == "pass" else "✗" if workflow.status == "fail" else "⚠"
        ...
        if workflow.missing_artifacts:
            print(f"  ❌ Missing Required Artifacts: {', '.join(workflow.missing_artifacts)}")
...
def main():
    ...
    for workflow_path in sorted(workflow_files):
        validation = validate_workflow(workflow_path, all_errors, all_warnings, all_notes)
