...
def main():
    ...
    if classified["merged"]:
        ...
        for b in classified["merged"]:
            print(
                f"     - {b.name} (last commit: {b.last_commit_date.strftime('%Y-%m-%d')})"
            )
    ...
    if classified["stale"]:
        ...
        for b in classified["stale"]:
            days = (
                b.last_commit_date.now(b.last_commit_date.tzinfo) - b.last_commit_date
            ).days
    ...
    report_path = (
        Path("analysis_reports") / f"branch_cleanup_{Path(__file__).stem}.json"
    )
