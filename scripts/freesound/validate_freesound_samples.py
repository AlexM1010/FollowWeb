    ...
    def validate_and_clean_checkpoint(
        ...
        with ProgressTracker(
            ...
            for i in range(0, len(sample_ids), batch_size):
                ...
                for sample_id in batch:
                    ...
                    progress.update(validated_count + len(samples_to_remove) + stats['api_errors'])
        ...
        if samples_to_remove:
            ...
            success_msg = EmojiFormatter.format(
                'success',
                f"All {stats['validated_samples']} samples are valid"
            )
...
def main() -> int:
    ...
    if not api_key:
        logger.error(
            EmojiFormatter.format(
                "error",
                "FREESOUND_API_KEY not found in environment"
            )
        )
    ...
    try:
        ...
        if loader.graph.number_of_nodes() == 0:
            logger.warning(
                EmojiFormatter.format(
                    'warning',
                    "No checkpoint found or checkpoint is empty"
                )
            )
            return 0
        ...
        if existing_checkpoint and 'metadata' in existing_checkpoint:
            validation_history = existing_checkpoint['metadata'].get('validation_history', {})
        ...
        logger.info(
            EmojiFormatter.format(
                'progress',
                "Saving checkpoint with updated validation timestamps..."
            )
        )
        ...
        logger.info(EmojiFormatter.format("chart", f"Total samples in checkpoint: {stats['total_samples']}"))
