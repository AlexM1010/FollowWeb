...
def get_most_downloaded_sample(
...
def get_checkpoint_aware_seed(
    loader, api_key: str, logger: logging.Logger
) -> tuple[int, str, int]:
    """
    ...
    if loader.graph.number_of_nodes() == 0:
        ...
        logger.info(
            "Checkpoint exists, selecting seed from SQL query (O(1) instant retrieval)..."
        )
        ...
        if best_seed:
            ...
            logger.warning(
                "No non-dormant samples found in SQL cache, fetching from API..."
            )
            return get_most_downloaded_sample(api_key, logger)
...
def main():
    ...
    logger.info(
        f"Edge generation: user={include_user_edges}, pack={include_pack_edges}, tag={include_tag_edges}"
    )
    ...
    try:
        ...
        if seed_sample_id is None:
            ...
            seed_sample_id, seed_name, seed_downloads = get_checkpoint_aware_seed(
                loader, api_key, logger
            )
        else:
        ...
        logger.info(
            EmojiFormatter.format(
                "success", f"Graph built: {final_nodes} nodes, {final_edges} edges"
            )
        )
        ...
        if skip_visualization:
            logger.info("=" * 70)
            logger.info(
                EmojiFormatter.format("completion", "Data Collection Complete!")
            )
            logger.info("=" * 70)
            logger.info(
                EmojiFormatter.format("chart", f"Seed sample ID: {seed_sample_id}")
            )
            if seed_name:
                logger.info(
                    EmojiFormatter.format("chart", f"Seed sample name: {seed_name}")
                )
            ...
            logger.info(
                EmojiFormatter.format(
                    "chart",
                    f"API requests used: {api_requests_used} / {max_requests} limit",
                )
            )
            logger.info(
                EmojiFormatter.format(
                    "info", "Visualization generation skipped (--skip-visualization)"
                )
            )
            logger.info("=" * 70)
            return 0
        ...
        output_prefix = (
            f"freesound_seed{seed_id_str}_{discovery_mode}_n{final_nodes}_{timestamp}"
        )
        ...
        if success:
            ...
            logger.error(
                EmojiFormatter.format("error", "Visualization generation failed")
            )
            return 1
