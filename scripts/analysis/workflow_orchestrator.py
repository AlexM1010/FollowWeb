...
class WorkflowOrchestrator:
    ...
    def __init__(
        ...
        self.conflict_matrix = {
            "freesound-nightly-pipeline": [
                "freesound-quick-validation",
                "freesound-full-validation",
            ],
            "freesound-quick-validation": [
                "freesound-nightly-pipeline",
                "freesound-full-validation",
    ...
    def check_workflow_status(self, workflow_name: str) -> Optional[Dict]:
        ...
        if self.dry_run:
            self.logger.info(
                f"[DRY RUN] Would check status of workflow: {workflow_name}"
            )
            return None
        ...
        if cache_key in self._status_cache:
            ...
            if age < self._cache_ttl:
                self.logger.debug(
                    f"Using cached status for {workflow_name} (age: {age:.1f}s)"
                )
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to check workflow status for {workflow_name}: {e}"
            )
            return None
    ...
    def wait_for_workflow(
        self, workflow_name: str, timeout: int = 7200, poll_interval: int = 30
    ) -> bool:
        """
        ...
        while time.time() - start_time < timeout:
            if not self.is_workflow_running(workflow_name):
                ...
                self.logger.info(
                    EmojiFormatter.format(
                        "success", f"{workflow_name} completed after {elapsed:.1f}s"
                    )
                )
                return True
        ...
        self.logger.warning(
            EmojiFormatter.format(
                "warning", f"Timeout waiting for {workflow_name} after {timeout}s"
            )
        )
        return False

    def check_and_wait_for_conflicts(
        self, current_workflow: str, timeout: int = 7200
        ...
        self.logger.info(
            EmojiFormatter.format(
                "info", f"Checking for conflicts with: {', '.join(conflicting)}"
            )
        )
        ...
        for workflow in conflicting:
            if self.is_workflow_running(workflow):
                ...
                self.logger.warning(
                    EmojiFormatter.format(
                        "warning", f"Detected running workflow: {workflow} ({run_url})"
                    )
                )
                ...
                if not completed:
                    ...
                    self.logger.warning(
                        EmojiFormatter.format(
                            "warning", f"SKIPPING EXECUTION: {reason}"
                        )
                    )
                    return False, reason
        ...
        self.logger.info(
            EmojiFormatter.format(
                "success", "No workflow conflicts detected, safe to proceed"
            )
        )
    ...
    def acquire_lock(self, lock_name: str, timeout: int = 300) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                self.logger.info(
                    EmojiFormatter.format("success", f"Acquired lock: {lock_name}")
                )
                return True
    ...
    def release_lock(self, lock_name: str) -> None:
        ...
        try:
            if lock_file.exists():
                lock_file.unlink()
                self.logger.info(
                    EmojiFormatter.format("success", f"Released lock: {lock_name}")
                )
