...
def check_milestone(checkpoint_dir: Path) -> Dict:
    ...
    if is_milestone:
        logger.info(
            f"🎉 Milestone {current_milestone} detected! ({current_nodes} nodes)"
        )
    else:
        logger.info(
            f"No milestone detected ({current_nodes} nodes, {current_nodes % 100} until next)"
        )
...
def main():
    ...
    if status["is_milestone"]:
        ...
        logger.info(
            f"Nodes since last milestone: {status['nodes_since_last_milestone']}"
        )
    else:
