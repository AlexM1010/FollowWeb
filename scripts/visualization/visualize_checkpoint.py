...
def load_graph(graph_path: Path) -> nx.DiGraph:
    ...
    if not isinstance(graph, (nx.Graph, nx.DiGraph)):
        raise TypeError(f"Expected NetworkX Graph or DiGraph, got {type(graph).__name__}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate visualization from checkpoint')
    parser.add_argument('--checkpoint-dir', required=True, help='Checkpoint directory')
    parser.add_argument('--output-dir', default='Output', help='Output directory')
    parser.add_argument('--renderer-type', default='sigma', help='Renderer type (sigma, pyvis, matplotlib)')
    ...
    try:
        ...
        logger.warning(f"Failed to load metadata (visualization will continue without it): {e}")
